...
async function submitLibraryModule() {
    ...
    try {
        await apiFetch(`/api/hr/courses/0/modules`, {
            method: 'POST',
...
async function openCourseContentModal(courseId) {
    ...
    if (course) {
        document.getElementById('contentModalTitle').textContent = 'Xây dựng nội dung: ' + (course.title || '');
    }
        ...
        data.modules.forEach(m => {
            ...
            <div class="structure-module">
                ...
                <div class="structure-module-body" id="drop-module-${m.moduleId}" ondragover="handleDragOver(event)" ondragleave="handleDragLeave(event)" ondrop="handleModuleDrop(event, ${m.moduleId})">
                    ...
                    `).join('') : '<div style="text-align:center; padding:15px; color:#cbd5e1; font-size:11px; border:1px dashed #f1f5f9; border-radius:6px;">Kéo bài học thả vào đây</div>'}
                </div>
            </div>`;
        });
    }
...
async function previewModuleLessons(moduleId) {
    try {
        const m = hrDocumentLibraryData.modules.find(x => x.moduleId === moduleId);
        const modalTitle = document.getElementById('previewLibraryTitle');
        const modalBody = document.getElementById('previewLibraryContent');
        if (!modalTitle || !modalBody) return;

        modalTitle.textContent = `Xem trước chương: ${m ? m.title : ''}`;
        openModal('previewLibraryModal');

        const lessons = hrDocumentLibraryData.lessons.filter(l => l.moduleId === moduleId);
        ...
        if (lessons && lessons.length) {
            lessons.forEach((l, idx) => {
                ...
                <div style="padding:12px; border:1px solid #cbd5e1; border-radius:6px; background:#fff;">
                    <div style="font-weight:700; color:#1e293b;">Bài ${idx+1}: ${hrLibraryEscape(l.title)}</div>
                    <div style="font-size:11px; color:#64748b; margin-top:2px;">Loại: ${l.contentType} ${l.level ? `• Level ${l.level}` : ''}</div>
                    ${(l.contentType === 'Text' || l.contentType === 'Document') && l.contentBody ? `<div style="margin-top:8px; font-size:12px; padding:10px; background:#f8fafc; border-radius:4px; max-height:150px; overflow-y:auto; border:1px solid #f1f5f9; white-space:pre-line;">${hrLibraryEscape(l.contentBody)}</div>` : ''}
                    ${l.videoUrl ? `<div style="margin-top:6px; font-size:12px; color:#2563eb;">▶️ Video: <a href="${l.videoUrl}" target="_blank">${hrLibraryEscape(l.videoUrl)}</a></div>` : ''}
                </div>`;
            });
        } else {
            html += '<div style="text-align:center; padding:12px; color:#94a3b8;">Chương này chưa có bài học nào.</div>';
        }
        html += '</div>';
...
async function previewLessonContent(lessonId) {
    try {
        const lesson = hrDocumentLibraryData.lessons.find(x => x.lessonId === lessonId);
        const modalTitle = document.getElementById('previewLibraryTitle');
        const modalBody = document.getElementById('previewLibraryContent');
        if (!modalTitle || !modalBody) return;

        modalTitle.textContent = `Xem trước bài học: ${lesson ? lesson.title : ''}`;
        openModal('previewLibraryModal');

        if (!lesson) {
            modalBody.innerHTML = '<div style="text-align:center; padding:12px; color:#ef4444;">Không tìm thấy bài học này.</div>';
            return;
        }
        
        let html = `<div style="display:flex; flex-direction:column; gap:16px;">
            <div>
                <span class="badge badge-purple" style="font-size:11px;">${lesson.contentType}</span>
                ${lesson.level ? `<span class="badge badge-info" style="font-size:11px; margin-left:6px;">Level ${lesson.level}</span>` : ''}
            </div>`;
        
        if (lesson.contentType === 'Text' || lesson.contentType === 'Document') {
            html += `<div style="padding:14px; background:#fff; border:1px solid #cbd5e1; border-radius:8px; line-height:1.6; white-space:pre-line; max-height:40vh; overflow-y:auto;">
                ${hrLibraryEscape(lesson.contentBody || 'Chưa có nội dung.')}
            </div>`;
            ...
            if (lesson.videoUrl) {
                if (lesson.videoUrl.includes('youtube.com') || lesson.videoUrl.includes('youtu.be')) {
                    ...
                    if (ytId) {
                        html += `<iframe style="width:100%; height:360px; border-radius:8px; border:none;" src="https://www.youtube.com/embed/${ytId}" allowfullscreen></iframe>`;
                    } else {
                        html += `<div style="font-size:13px; color:#2563eb;">▶️ Link Video: <a href="${lesson.videoUrl}" target="_blank">${hrLibraryEscape(lesson.videoUrl)}</a></div>`;
                    }
                } else {
                    html += `<video style="width:100%; border-radius:8px; background:#000;" src="${lesson.videoUrl}" controls></video>`;
                }
            } else {
                html += '<div style="text-align:center; padding:12px; color:#ef4444; border:1px dashed #fee2e2; border-radius:8px; background:#fef2f2;">Lỗi: Bài học video nhưng chưa được gán file hay đường dẫn video.</div>';
            }
        }

        if (lesson.attachments && lesson.attachments.length > 0) {
            html += `<div>
                <div style="font-weight:700; color:#1e293b; font-size:13px; margin-bottom:8px;">Tài liệu đính kèm:</div>
                <div style="display:flex; flex-direction:column; gap:6px;">
                    ${lesson.attachments.map(a => `
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 12px; border:1px solid #cbd5e1; border-radius:6px; background:#f8fafc;">
                            <span style="font-size:12px; font-weight:600; color:#475569;">📎 ${hrLibraryEscape(a.fileName || 'Link tài liệu')}</span>
                            <div style="display:flex; gap:6px;">
                                <a class="btn btn-secondary btn-sm" style="padding:2px 8px; font-size:11px;" href="${a.filePath}" target="_blank">Tải xuống</a>
                                <button class="btn btn-danger btn-sm" style="padding:2px 8px; font-size:11px;" onclick="deleteAttachment(${a.attachmentId}, ${lessonId})">Xóa</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
        ...
        showToast('Không tải được chi tiết bài học: ' + e.message, 'error');
    }
}
